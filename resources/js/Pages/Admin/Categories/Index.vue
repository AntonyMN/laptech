...
const submit = () => {
    if (editingCategory.value) {
        form.patch(route('admin.categories.update', editingCategory.value.id), {
...
const deleteCategory = (id) => {
    if (confirm('Are you sure?')) {
        form.delete(route('admin.categories.delete', id));
    }
};
</script>
                ...
                <div class="bg-charcoal-light border border-white/5 rounded-[2.5rem] p-10 sticky top-12">
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea v-model="form.description" rows="3" class="w-full bg-charcoal border-white/5 rounded-2xl p-4 focus:border-red focus:ring-red transition"></textarea>
                        </div>
                    ...
                    <table class="w-full text-left">
                        <thead>
                            <tr class="bg-white/5 text-xs uppercase tracking-widest text-white/40 font-bold">
                                ...
                                <th class="px-10 py-6">Products</th>
                        ...
                        <tbody class="divide-y divide-white/5">
                            <tr v-for="category in categories" :key="category.id" class="hover:bg-white/5 transition group">
                                ...
                                <td class="px-10 py-6">
                                    <span class="px-3 py-1 bg-white/5 rounded-full text-xs font-bold">{{ category.products_count }} items</span>
                                </td>
